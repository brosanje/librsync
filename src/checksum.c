...
void rs_calc_strong_sum(void const *buf, size_t buf_len, rs_strong_sum_t *sum)
{
    rs_calc_strong_sum_magic(buf, buf_len, sum, RS_DEFAULT_SIG_MAGIC);
}

int rs_calc_strong_sum_magic(void const *buf, size_t buf_len, rs_strong_sum_t *sum
    , int magic)
{
    /* Lazy calculate strong sum after finding weak match. */
    if (RS_BLAKE2_SIG_MAGIC == magic) {
        rs_calc_blake2_sum(buf, buf_len, sum);
        return 1;
    }

    if (RS_MD4_SIG_MAGIC == magic) {
        rs_calc_md4_sum(buf, buf_len, sum);
        return 1;
    }

    /* Bad input data is checked in rs_delta_begin, so this
    * should never be reached. */
    rs_fatal("Unknown signature algorithm %#x", magic);
    return 0;
}
